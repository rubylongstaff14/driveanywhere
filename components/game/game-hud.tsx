...
export function GameHud({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 top-0 z-10 select-none p-3 sm:p-4">
      <div className="mobile-race-hud mx-auto items-center justify-between gap-2 rounded-xl border border-white/15 bg-black/72 px-3 py-1.5 shadow-xl backdrop-blur-md">
        <div className="min-w-0">
          <p className="max-w-36 truncate font-mono text-[8px] uppercase tracking-wider text-white/55">
            {routeName}
          </p>
          <p className="font-mono text-lg font-bold leading-none tabular-nums text-white">
            {started ? formatLapTime(elapsedMs) : "READY"}
          </p>
        </div>
        <div className="text-center">
          <p className="font-mono text-2xl font-black leading-none tabular-nums text-accent-bright">
            {Math.round(speedKph)}
          </p>
          <p className="font-mono text-[7px] uppercase tracking-widest text-white/50">
            km/h · G{gear <= 0 ? "N" : gear}
          </p>
        </div>
      ...
      {/* ---- Streak pill ---- */}
      {streak >= 2 && (
        <div className="desktop-race-hud absolute top-3 left-1/2 z-10 -translate-x-1/2">
      ...
      <div className="desktop-race-hud mx-auto flex max-w-7xl items-start justify-between gap-2">
        {/* ---- Timer block ---- */}
        <div
      ...
      {isTournamentRace && tournamentCurrentRound && (
        <div className="desktop-race-hud mt-2 flex justify-start">
