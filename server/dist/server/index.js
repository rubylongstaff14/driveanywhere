...
import { paintHexesForVehicle, stockIdsFor, } from "../lib/game/cosmetics";
        ...
        switch (msg.type) {
            ...
            case "host_set_vehicle": {
                ...
                if (!player?.isHost || room.status !== "waiting")
                    break;
                if (msg.type === "host_set_map")
                    room.map = msg.map;
                if (msg.type === "host_set_difficulty")
            ...
            case "taunt": {
                ...
                if (!player)
                    break;
                const now = Date.now();
                if (now - player.lastTauntAt < 2800)
                    break;
                if (!isTauntId(msg.tauntId))
                    break;
            ...
            case "join_next_race": {
                ...
                if (!info)
                    break;
                const room = rooms.get(info.roomId);
                if (!room || (room.status !== "waiting" && room.status !== "results"))
                    break;
                const player = room.players.find((p) => p.id === info.playerId);
...
const defaultServers = [
    { name: "Westminster Sprint — Open Lobby", map: "westminster-sprint", difficulty: "medium" },
    { name: "Dubai Marina — All Welcome", map: "dubai-marina-circuit", difficulty: "medium" },
    { name: "New York Harbor — Competitive", map: "new-york-harbor-circuit", difficulty: "hard" },
];
