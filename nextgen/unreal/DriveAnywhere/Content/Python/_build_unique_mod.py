...
out = Path("da_landmark_unique.py")
# Read part1 if exists else empty
p1 = Path("_gen_unique_part1.py")
base = p1.read_text(encoding="utf-8") if p1.exists() else ""

rest = r'''
def _elizabeth_tower(sp, x, y, z, h, stone, dark, gold, white, copper):
    sp.box(unreal.Vector(x, y, z + h * 0.04), 16.5, 16.5, h * 0.08 / 100, dark, True)
    sp.box(unreal.Vector(x, y, z + h * 0.22), 14.2, 14.2, h * 0.28 / 100, stone, True)
    for ox, oy in ((-6.5, -6.5), (6.5, -6.5), (-6.5, 6.5), (6.5, 6.5)):
        sp.box(unreal.Vector(x + ox * 100, y + oy * 100, z + h * 0.22), 1.4, 1.4, h * 0.26 / 100, dark)
    sp.box(unreal.Vector(x, y, z + h * 0.48), 12.2, 12.2, h * 0.28 / 100, stone, True)
    sp.box(unreal.Vector(x, y, z + h * 0.68), 13.5, 13.5, h * 0.12 / 100, white)
    for yaw, ox, oy in ((0, 0, 7.0), (90, 7.0, 0), (180, 0, -7.0), (270, -7.0, 0)):
        sp.box(unreal.Vector(x + ox * 100, y + oy * 100, z + h * 0.68), 0.3, 6.5, 6.5, gold, False, yaw)
    sp.box(unreal.Vector(x, y, z + h * 0.80), 7.2, 7.2, h * 0.1 / 100, dark)
    sp.coni(unreal.Vector(x, y, z + h * 0.92), 4.8, h * 0.16 / 100, copper)
    sp.cyli(unreal.Vector(x, y, z + h * 0.995), 0.25, h * 0.04 / 100, gold)
...
def _london_eye(sp, x, y, z, h, steel, accent):
    r = h * 0.42
    hub = z + h * 0.55
    sp.cyli(unreal.Vector(x, y, hub), 2.5, 2.5, steel, True)
    for i in range(16):
        ang = math.radians(i * 22.5)
        mx = x + math.cos(ang) * r * 0.5
        mz_off = math.sin(ang) * r * 0.5
        sp.box(unreal.Vector(mx, y, hub + mz_off), 0.4, 0.4, (r * 0.95) / 100, steel, False, i * 22.5)
        cx = x + math.cos(ang) * r
        cz = hub + math.sin(ang) * r
        sp.box(unreal.Vector(cx, y, cz), 2.2, 3.5, 2.2, accent)
    sp.box(unreal.Vector(x - 900, y, z + h * 0.2), 1.5, 1.5, h * 0.45 / 100, steel, True, 18)
    sp.box(unreal.Vector(x + 900, y, z + h * 0.2), 1.5, 1.5, h * 0.45 / 100, steel, True, -18)
...
def _parliament(sp, x, y, z, h, stone, dark):
    sp.box(unreal.Vector(x, y, z + h * 0.35), 95.0, 22.0, h * 0.65 / 100, stone, True, 25)
    for i in range(7):
        ox = (i - 3) * 1200
        sp.box(unreal.Vector(x + ox, y + 900, z + h * 0.55), 6.0, 6.0, h * 0.35 / 100, dark)
    sp.box(unreal.Vector(x + 4500, y - 800, z + h * 0.55), 16.0, 16.0, h * 1.05 / 100, stone, True)
    sp.box(unreal.Vector(x + 4500, y - 800, z + h * 1.12), 18.0, 18.0, h * 0.08 / 100, dark)

def _abbey(sp, x, y, z, h, stone, lead):
    sp.box(unreal.Vector(x, y, z + h * 0.4), 32.0, 70.0, h * 0.75 / 100, stone, True, -90)
    sp.box(unreal.Vector(x, y, z + h * 0.82), 28.0, 62.0, h * 0.08 / 100, lead, False, -90)
    for ox in (-1100, 1100):
        sp.box(unreal.Vector(x + ox, y - 2200, z + h * 0.55), 11.0, 11.0, h * 0.95 / 100, stone, True)
        sp.box(unreal.Vector(x + ox, y - 2200, z + h * 1.05), 12.0, 12.0, h * 0.08 / 100, lead)
...
def _generic_varied(sp, name, kind, x, y, z, h, paint, accent):
    ...
    if kind in ("pyramid", "sugarloaf", "ziggurat"):
        ...
        return
    if kind == "ferris":
        r = h * (0.35 + (hv % 10) / 100.0)
        hub = z + h * 0.5
        spokes = 8 + (hv % 8)
        sp.cyli(unreal.Vector(x, y, hub), 1.5, 1.5, paint, True)
        for i in range(spokes):
            ang = math.radians(i * (360.0 / spokes))
            sp.box(unreal.Vector(x + math.cos(ang) * r * 0.5, y, hub + math.sin(ang) * r * 0.5),
                   0.35, 0.35, r / 100, paint, False, i * (360.0 / spokes))
        return
    if kind == "sail":
        lean = 8 + (hv % 15)
    ...
    if kind in ("bridge-tower", "pylon-gate", "torii", "gold-frame"):
        gap = 8 + (hv % 12)
        for side in (-1, 1):
            sp.box(unreal.Vector(x + side * gap * 100, y, z + h * 0.5), 3 + (hv % 4), 3 + (hv % 3), h / 100, paint, True)
        sp.box(unreal.Vector(x, y, z + h * (0.7 + (hv % 20) / 100.0)), gap * 2.2, 2, h * 0.08 / 100, accent)
        return
...
def build_landmark(name, kind, tag, x, y, z, h_cm, paint, accent, spawn_fn, mats=None):
    ...
    if "elizabeth tower" in key or "big ben" in key:
    ...
    if "london eye" in key or key == "eye wheel court":
        _london_eye(sp, x, y, z, max(h, 12000), steel, accent); return
    ...
    if "palace of westminster" in key:
        _parliament(sp, x, y, z, max(h, 9000), stone, dark); return
    if "westminster abbey" in key:
    ...
    if "victoria tower" in key:
        sp.box(unreal.Vector(x, y, z + h * 0.45), 14, 14, h * 0.88 / 100, stone, True)
        sp.box(unreal.Vector(x, y, z + h * 0.95), 16, 16, h * 0.1 / 100, dark); return
    if "mi6" in key:
        for i, (w, frac) in enumerate([(28, 0.35), (22, 0.65), (16, 0.9)]):
            prev = 0 if i == 0 else [0.35, 0.65][i - 1]
            mid = z + h * (prev + frac) * 0.5
            sp.box(unreal.Vector(x, y, mid), w, w * 0.85, h * (frac - prev) / 100, paint, i == 0)
        return
    if "st paul" in key:
    ...
    if "ain dubai" in key or "cosmo wheel" in key or "wonder wheel" in key:
        _london_eye(sp, x, y, z, h, steel, accent); return

    _generic_varied(sp, name, kind, x, y, z, h, paint, accent)
'''

out.write_text(base + rest, encoding="utf-8")
